  ...
  'comments|5': [
    {
      ...
      content: '@sentence',
      avatar () {
        return mockjs.Random.image('48x48', mockjs.Random.color(), '#757575', 'png', this.name.substr(0, 1))
      },
      date () {
        return `2016-${mockjs.Random.date('MM-dd')} ${mockjs.Random.time('HH:mm:ss')}`
      },
    },
  ],
  'recentSales|36': [
    {
      ...
      'status|1-4': 1,
      date () {
        return `${mockjs.Random.integer(2015, 2016)}-${mockjs.Random.date('MM-dd')} ${mockjs.Random.time('HH:mm:ss')}`
      },
      'price|10-200.1-2': 1,
    },
  ],
