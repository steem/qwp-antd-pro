...
function is_json_content_type_request() {
    return starts_with(get_request_content_type(), 'application/json');
}
...
function initialize_request() {
    ...
    $OP = P('op');
    if (is_post_request()) {
        if (is_json_content_type_request()) {
            $_POST = json_from_string(@file_get_contents('php://input'));
...
function PP($name, $default = null, &$src_arr = null) {
  if ($src_arr && isset($src_arr[$name])) {
      return $src_arr[$name];
  }
...
function PA(&$names, $default = null, &$src_arr = null) {
    foreach ($names as $name) {
        $v = P($name, $default, $src_arr);
        if ($v) return $v;
    }
    return $default;
}
...
function CS($name, &$new_value = null) {
    global $_SESSION;
...
function save_uploaded_file($form_name, $file_name = null, $save_path = null, $keep_ext = true) {
    ...
    $item = &$_FILES[$form_name];
    if (!$save_path) {
        $save_path = dirname($item["tmp_name"]);
    }
    $name = &$item["name"];
    if (!$file_name) {
        $file_name = $name;
        ...
        if ($keep_ext) {
            $pos = strrpos($name, '.');
            if ($pos !== false) {
                $file_name .= strtolower(substr($name, $pos));
            }
        }
    }
    $file_path = join_paths($save_path, $file_name);
    if (move_uploaded_file($item["tmp_name"], $file_path) === true) {
        return array($name, $file_name, $file_path, $item['type'], $item['size']);
    }
    return false;
}
...
function get_current_datetime_string() {
    return get_datetime_string(time());
}
