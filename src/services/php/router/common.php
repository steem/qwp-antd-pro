...
define('MODULE_NAME_RULE', '/^[\w-_]+$/');

function qwp_set_form_validator($name, $op_tag = null) {
    global $QWP_FORM_VALIDATOR_RULE, $QWP_FORM_OP_TAG;
    $QWP_FORM_VALIDATOR_RULE = $name;
    $QWP_FORM_OP_TAG = $op_tag;
}
function qwp_set_file_validator($name) {
    global $QWP_FILE_VALIDATOR_RULE;
    $QWP_FILE_VALIDATOR_RULE = $name;
}
function qwp_set_search_validator($name) {
    global $QWP_SEARCH_FORM_VALIDATOR_RULE;
    $QWP_SEARCH_FORM_VALIDATOR_RULE = $name;
}
...
function qwp_set_ops_process($fn, $transaction = false) {
...
function qwp_is_module_name_valid() {
    ...
    foreach ($MODULE as $item) {
        if (!preg_match(MODULE_NAME_RULE, $item)) {
            return false;
        }
    }
    return true;
...
function qwp_render_get_form_rules($root, $prefix, &$app_settings) {
    ...
    foreach ($files as $item) {
        if (starts_with($item, $prefix) && (ends_with($item, '_validator.php') || ends_with($item, '_search.php'))) {
            $form_rule = null;
            require($root . '/' . $item);
...
function qwp_render_app_settings(&$app_settings = null, $m = null)
{
    ...
    if (!isset($app_settings['lang'])) $app_settings['lang'] = array();
    ...
    if ($MODULE) {
        ...
        if ($PAGE) $prefix = $prefix . $PAGE . '_';
        qwp_render_get_form_rules($app_root, $prefix, $app_settings);
    }
    qwp_create_and_echo_json_response(true, false, 'success', $app_settings);
}
function qwp_check_request() {
  global $MODULE, $PAGE, $OP, $MODULE_URI, $IS_APP_SETTING_REQ;

  $IS_APP_SETTING_REQ = $OP === QWP_APP_SETTINGS_OP;
  // check module parameters
  if (!$MODULE) {
    if (QWP_JUST_SERVICE) {
        return $IS_APP_SETTING_REQ;
    }
    $MODULE = DEFAULT_MODULE;
  }
  if ($PAGE && !preg_match(MODULE_NAME_RULE, $PAGE)) {
    return false;
  }
  if ($OP) {
    if(!$IS_APP_SETTING_REQ  && !preg_match(MODULE_NAME_RULE, $OP)) {
      return false;
    }
  }
  $MODULE_URI = $MODULE;
...
function qwp_initialize() {
    ...
    $USER = null;
    $MODULE_ROOT = join_paths(QWP_MODULE_ROOT, $MODULE_URI);
...
function qwp_set_upload_file_result(&$data, &$file_info = null, &$error = null) {
    if (!$file_info) {
        $data['status'] = 'error';
        return false;
    }
